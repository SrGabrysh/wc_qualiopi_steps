...
(function($) {
    ...
    const LogViewer = {
        ...
        bindEvents: function() {
            // Filtres
            $('#wcqs-time-filter, #wcqs-level-filter, #wcqs-source-filter').on('change', this.loadLogs.bind(this));
        ...
        displayLogs: function(response) {
            ...
            if (!logs || logs.length === 0) {
                $content.html('<div class="wcqs-no-logs">Aucun log trouvé pour les critères sélectionnés</div>');
                return;
            }
        ...
        createLogEntry: function(log) {
            ...
            return $(`
                <div class="wcqs-log-entry ${levelClass} ${sourceClass}">
                    <span class="wcqs-log-time">${log.datetime}</span>
                    <span class="wcqs-log-level">
                        <span class="wcqs-level-badge wcqs-level-${log.level}">${log.level.toUpperCase()}</span>
                    </span>
                    <span class="wcqs-log-source">${this.formatSource(log.source)}</span>
                    <span class="wcqs-log-message">${this.formatMessage(log.message)}</span>
                </div>
            `);
        },
        ...
        formatMessage: function(message) {
            ...
            return message
                ...
                .replace(/(ERROR|FATAL|CRITICAL)/g, '<span class="wcqs-highlight-error">$1</span>')
                .replace(/(WARNING|WARN)/g, '<span class="wcqs-highlight-warning">$1</span>')
                .replace(/(SUCCESS|OK|PASS)/g, '<span class="wcqs-highlight-success">$1</span>');
        },
            ...
            $.post(wcqsLogViewer.ajax_url, data)
                .done((response) => {
                    if (response.success) {
                        ...
                        this.showError(response.data?.message || 'Erreur lors de la suppression');
                    }
                })
                .fail(() => {
        ...
        showModal: function(title, content) {
            ...
            // Fermeture
            $modal.on('click', '.wcqs-modal-close, .wcqs-modal-overlay', function(e) {
                if (e.target === this) {
                    $modal.remove();
                }
            });
